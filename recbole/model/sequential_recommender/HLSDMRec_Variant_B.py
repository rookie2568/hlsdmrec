...
class GARec_GRU(SequentialRecommender):
    def __init__(self, config, dataset):
        super(GARec_GRU, self).__init__(config, dataset)
        ...
        self.contrastive_temp = config.get('contrastive_temp', 0.1)
    ...
    def conv1d_gru_encoder(self, short_term_seq):
    ...
    def forward(self, item_seq, item_seq_len, user_id):
        ...
        if short_term_item.size(1) > 0:
            short_term_item_encoded = self.conv1d_gru_encoder(short_term_item)
            short_term_feature_encoded = self.conv1d_gru_encoder(short_term_feature)
        ...
        item_weights = self.item_gate(item_gate_input)

        feature_gate_input = torch.cat([user_emb, long_term_feature_final, short_term_feature_final], dim=-1)
        feature_weights = self.feature_gate(feature_gate_input)
