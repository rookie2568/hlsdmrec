...
class GARec(SequentialRecommender):
    ...
    def forward(self, item_seq, item_seq_len, user_id):
        ...
        # Modified: Long-term sequence now includes the entire sequence (n elements)
        # while short-term sequence remains the last q elements, creating overlap
        short_term_start = max(0, seq_len - self.q)

        # Long-term: entire sequence [0, seq_len) - capture full user behavior history
        long_term_item = input_emb  # Use the entire sequence
        long_term_feature = feature_emb  # Use the entire sequence

        # Short-term: last q elements [short_term_start, seq_len) - capture recent preferences
        short_term_item = input_emb[:, short_term_start:, :]
        short_term_feature = feature_emb[:, short_term_start:, :]

        # Long-term encoding with FFT + Mamba - processes the entire sequence
        long_term_item_encoded = self.fft_ifft_mamba(long_term_item)
        long_term_feature_encoded = self.fft_ifft_mamba(long_term_feature)
        # Extract the final hidden state from the full sequence
        long_term_item_final = self.gather_indexes(long_term_item_encoded,
                                                   torch.full((batch_size,), seq_len - 1,
                                                              dtype=torch.long, device=item_seq.device))
        long_term_feature_final = self.gather_indexes(long_term_feature_encoded,
                                                      torch.full((batch_size,), seq_len - 1,
                                                                 dtype=torch.long, device=item_seq.device))

        # Short-term encoding with Conv1D + LSTM - processes only the last q elements
        if short_term_item.size(1) > 0:
            ...
            # Extract the final hidden state from the short sequence
            ...
            # Handle edge case where sequence length is too short for short-term modeling
        ...
        # Contrastive learning losses remain the same
