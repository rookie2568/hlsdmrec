...
class MInterest(SequentialRecommender):
    r"""
    ...
    def __init__(self, config, dataset):
        ...
        super(MInterest, self).__init__(config, dataset)
        ...
        self.relu = nn.ReLU()
        self.caps_num = 5
        self.caps_linear = nn.Linear(self.caps_num, self.hidden_size)
        self.k = 3
        # 胶囊网络
    ...
    def forward(self, seq_item, user, seq_item_len):
        ...
        transition_output = item_trm_output[-1]
        # transition_output = self.LayerNorm(transition_output)
        ...
        long_term_item_pooling_layer = self.long_term_item_pooling_layer(transition_output, user_embedding, mask)

        short_item_embedding = transition_output[:, -self.short_item_length:, :]
